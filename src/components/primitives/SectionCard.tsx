    ...
    >
      <header className="mb-6 flex items-start justify-between gap-4 border-b border-[var(--m3-outline)] pb-4">
        <div>
          ...
          {subtitle ? (
            <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--m3-on-surface-variant)] md:text-base">{subtitle}</p>
          ) : null}
        </div>
        {icon ? <div className="text-[var(--m3-primary)] opacity-75">{icon}</div> : null}
      </header>
      {children}
    </section>
  );
